...
def get_questions() -> List[Question]:
    ...
    if os.path.exists(QUESTIONS_JSON_PATH):
        print(f"Loading questions from cache: {QUESTIONS_JSON_PATH}")
        with open(QUESTIONS_JSON_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    
    # Otherwise, fetch from API, process files, and cache
    try:
        response = requests.get(f"{BASE_RUL}/questions", timeout=15)
        ...
        for i, api_question in enumerate(api_questions, 1):
            ...
            question = Question(question=question_text, file_path=file_path)
        ...
        print(f"Cached processed questions to {QUESTIONS_JSON_PATH}")
...
def get_random_question() -> Question:
    ...
    """
    try:
        response = requests.get(f"{BASE_RUL}/random-question", timeout=15)
        response.raise_for_status()
...
def get_file(task_id: str, file_name: str) -> str:
    ...
    try:
        response = requests.get(f"{BASE_RUL}/files/{task_id}", timeout=30)
        response.raise_for_status()
...
def submit_answers(username: str, agent_code: str, answers: List[Dict[str, Union[str, int, float]]]) -> Dict[str, Any]:
    ...
    try:
        response = requests.post(f"{BASE_RUL}/submit", json=submission_data, timeout=60)
        response.raise_for_status()
